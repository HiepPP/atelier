...
enum GitServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidOutput:
            return "Git returned invalid text output."
        }
    }
}
...
final class GitCommand: @unchecked Sendable {
    private let lock = NSLock()
...
final class GitService: @unchecked Sendable {
    ...
    func unstage(path: String, originalPath: String?, workspacePath: String) async throws {
        ...
        if await hasHead(workspacePath: workspacePath) {
            _ = try await run(
    ...
    private func hasHead(workspacePath: String) async -> Bool {
        do {
            _ = try await run(
