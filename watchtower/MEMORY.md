...
- 2026-07-31: One Markdown renderer now serves both surfaces. `MarkdownAttributedDocumentBuilder.build` takes an `AgentMarkdownPresentation`, and `AgentMarkdownView` hosts a bare `NSTextView` with no inner `NSScrollView`. Do not add a second Markdown code path. Add a mode to the builder instead.
- 2026-07-31: When text is inserted into an attributed document, a region whose location equals the insert location must move with it. `MarkdownRegionShift.shifted` guarded on `range.location > location` and left the very next block pinned to the inserted text, so a code copy control landed on the wrong row. Use `>=` and add a boundary case to the shift test, because a test with only far-before and far-after cases never sees it.
- 2026-07-31: `AtelierGhostButtonStyle` already ends in `.atelierPointerCursor()` at [app/Atelier/Sources/Atelier/Theme/AtelierComponents.swift](app/Atelier/Sources/Atelier/Theme/AtelierComponents.swift) line 454. Never write a Verify line that greps a file for `atelierPointerCursor`. A control that uses the style is correct and greps empty, so the check reads as a failure when the rule is met.
- 2026-07-31: No `computer-use` plugin is installed on this machine. A TASK Prompt naming `$computer-use:computer-use` cannot run, so plan any on-screen acceptance as BLOCKED-until-installed and record what CLI evidence does exist. `atelier-doctor` `cpuPercent` beats a `ps` window right after launch: the first 20 seconds carry startup spikes to 8.8 percent that are not idle cost.
- 2026-07-31: `NSParagraphStyle.lineSpacing` is extra room added on top of the font's own line height, so an absolute value is not a rhythm. In the Markdown builder a flat 8 points produced a prose line-height ratio of 1.909 at transcript scale 0.8 and 1.438 at document zoom 2.0. Express line spacing as a target ratio and back-solve `pointSize * ratio - lineHeight`. `MarkdownTypeScale` in [app/Atelier/Sources/Atelier/Editor/MarkdownSelectableDocumentView.swift](app/Atelier/Sources/Atelier/Editor/MarkdownSelectableDocumentView.swift) owns it, and `MarkdownRhythm` carries it so no helper needs a new parameter. The same trap applies to any `max()` floor on a scaled font size: a clamp changes the hierarchy's shape at the small end of the range.
- 2026-07-31: The agent response panel renders one card at a time (`if let response = selectedResponse`), not a stack. A plan that assumes stacked cards will invent cross-card wiring that no code needs. Read the panel body before planning anything that depends on how many responses are on screen.
- 2026-07-31: `.atelierFont` resolves its size through `\.atelierZoomScale`, and the response transcript sets that value to `zoomScale * textScale`. Every `.atelierFont` inside the transcript already scales with the user's text-size control. Do not plan a manual scale multiply for text inside that container, and do not claim a fixed base size renders at its base value there.
- 2026-07-31: A passing test suite says nothing about whether a surface reads well. Two rounds of Markdown work shipped green, and every real defect came from capturing the window and sampling pixels: a 21.5 point measure overhang, an inline code fill at `#D5D0C9` on a `#F8F7F4` page, and five header buttons at full accent. Prove a visual change by sampling the pixel before and after, and put that sample in the outcome.
- 2026-07-31: `.buttonStyle(.glass)` picks up the application accent tint, so an enabled control renders as a solid terracotta block. It is not a neutral style. A disabled one looks quiet, which hides the problem in a screenshot. Use `AtelierGhostButtonStyle()` for quiet chrome; it already ends in `.atelierPointerCursor()`, so drop any explicit call beside it.
- 2026-07-31: Never gate a sibling's visibility on a width you measure from the view that sibling shrinks. Showing the outline rail based on the scroll view's width oscillates: the rail narrows the scroll view, which pushes the width back under the threshold. Measure the container that holds both.
- 2026-07-31: Removing a decoration can delete a whole class of bug instead of fixing it. Dropping the inline code fill fixed the mosaic, the detached punctuation from the 12 point clearance kern, and the wrapped-path chip fragments in table cells, all at once. Before tuning a decoration's colors, ask whether the decoration is needed.
